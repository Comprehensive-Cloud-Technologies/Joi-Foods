...
class Departments extends CI_Controller
{
    ...
    public function index($company_id)
    {
        if (is_loggedin_client()) {
            ...
            $this->load->view('client/departments/index', $data);
            $this->load->view('client/common/footer', $footer_data);

            $this->load->view('client/validation/departments');
        } else {
    ...
    public function add($company_id)
    {
        if (is_loggedin_client()) {
            ...
            $this->load->view('client/common/header', $header_data);
            $this->load->view('client/common/sidebar');
            $this->load->view('client/departments/add', $data);
            $this->load->view('client/common/footer', $footer_data);

            $this->load->view('client/validation/departments_form');
        } else {
            $data['title'] = 'Login || ' . config_item('application_name');
            $this->load->view('client/auth/login', $data);
        }
    ...
    public function store()
    {
        if (is_loggedin_client()) {
            ...
            if (isset($post_data['company_id']) && $post_data['company_id'] !== '' && !empty($post_data['name'])) {
                ...
                if ($this->departments->check_name_exists($post_data['company_id'], $post_data['name'])) {
                    echo json_encode(['status' => 400, 'message' => 'Department name already exists']);
                    return;
                }

                // Check if code already exists (if provided)
                if (!empty($post_data['code']) && $this->departments->check_code_exists($post_data['company_id'], $post_data['code'])) {
    ...
    public function edit($id)
    {
        if (is_loggedin_client()) {
            ...
            // Verify company belongs to this client
            $company = $this->companies->get_by_id($department->company_id, $client_id);
            if (!$company) {
                $this->session->set_flashdata('error', 'Company not found');
                redirect('client/companies');
            }
            ...
            $this->load->view('client/common/header', $header_data);
            $this->load->view('client/common/sidebar');
            $this->load->view('client/departments/edit', $data);
            $this->load->view('client/common/footer', $footer_data);
    ...
    public function update()
    {
        if (is_loggedin_client()) {
            ...
            if (isset($post_data['department_id']) && $post_data['department_id'] !== '' && !empty($post_data['name'])) {
                ...
                if ($this->departments->check_name_exists($department->company_id, $post_data['name'], $post_data['department_id'])) {
                    echo json_encode(['status' => 400, 'message' => 'Department name already exists']);
                    return;
                }

                // Check if code already exists (if provided)
                if (!empty($post_data['code']) && $this->departments->check_code_exists($department->company_id, $post_data['code'], $post_data['department_id'])) {
                    echo json_encode(['status' => 400, 'message' => 'Department code already exists']);
                    return;
                }
