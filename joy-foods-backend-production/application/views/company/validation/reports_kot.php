...
function loadKotData() {
    ...
    $.ajax({
        url: base_url + 'company/reports/kot_data',
        type: 'POST',
        data: {
...
function updateTable(orders) {
    ...
    $.each(orders, function(index, order) {
        ...
        kotTable.row.add([
            ...
            typeBadge,
            order.department_name || '<span class="text-muted">-</span>',
            order.location_name || '<span class="text-muted">-</span>',
            ...
            '<span class="badge bg-' + color + '-subtle text-' + color + '">' + order.status + '</span>'
        ]);
    });
...
function renderOrderDetail(order, items, payments) {
    ...
    html += '<span class="badge bg-warning-subtle text-warning me-1">KOT</span>';
